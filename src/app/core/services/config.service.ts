...
export class CoreConfigService {
	...
	private _configSubject: BehaviorSubject<any> = new BehaviorSubject(undefined);

	constructor(
		private _router: Router,
		@Inject(CORE_CUSTOM_CONFIG) private _config: CoreConfig,
		private localStorageService: LocalStorageService
	) {
		// Get the config from local storage
		if (_config.layout.enableLocalStorage) {
			this.localConfig = this.localStorageService.getItem('config');
		} else {
			this.localStorageService.removeItem('config');
		}

		// Set the defaultConfig to localConfig if we have else appConfig (app-config.ts)
		this._defaultConfig =
			this.localConfig && Object.keys(this.localConfig).length !== 0
				? this.localConfig
				: _config;
	...
	set config(data) {
		...
		if (this.localConfig && Object.keys(this.localConfig).length !== 0) {
		...
		if (config.layout.enableLocalStorage) {
			this.localStorageService.setItem('config', config);
		}
	...
	setConfig(data: any, param = { emitEvent: true }): void {
		...
		if (
			!_.isEqual(this._configSubject.getValue().app, data.app) &&
			this._defaultConfig.layout.enableLocalStorage &&
			!config.layout.enableLocalStorage
		) {
			let configTemp = _.cloneDeep(config);
			configTemp.layout = _.cloneDeep(this._defaultConfig.layout);
			this.localStorageService.setItem('config', configTemp);
		}
